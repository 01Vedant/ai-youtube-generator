...
export default function MarketplacePage(): JSX.Element {
  ...
  const initialSort = sp.get('sort') === 'popular' ? 'popular' : 'new';
  const [sort, setSort] = useState<'new' | 'popular'>(initialSort);
  const [page, setPage] = useState(Number(sp.get('page') || 1) || 1);
  const [pageSize, setPageSize] = useState(Number(sp.get('page_size') || 24) || 24);
  ...
  const [schema, setSchema] = useState<InputsSchema | undefined>(undefined);
  ...
  }, [q, category, sort, page, pageSize, navigate]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setSchema(full.inputs_schema || undefined);
  ...
  const handleDuplicate = async (tplId: string) => {
    if (!user) {
      navigate(`/login?next=${encodeURIComponent('/marketplace' + (location.search || ''))}`);
      return;
    }
    try {
      await duplicateTemplate(tplId);
      ...
      {flags && flags.marketplace === false ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            ...
            <select value={sort} onChange={(e) => setSort((e.target.value as 'new' | 'popular') || 'new')}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
                {items.map((tpl) => (
                  <div key={tpl.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
                    ...
                    >
                      {tpl.thumb ? (
                        <img src={tpl.thumb} alt="thumb" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ) : (
                    ...
                    <div style={{ padding: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                        ...
                        <button onClick={() => void handleDuplicate(tpl.id)}>Duplicate</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)' }} onClick={() => setSelected(null)}>
              <div style={{ background: '#fff', maxWidth: 640, margin: '10vh auto', padding: 24, borderRadius: 8 }} onClick={(e) => e.stopPropagation()}>
                ...
                <TemplateQuickFill
                  vars={vars}
                  schema={schema}
                  onPreview={async (inputs): Promise<{ warnings?: string[] }> => {
                ...
                />
                {!user && <div style={{ marginTop: 8, fontSize: 12, color: '#6b7280' }}>Sign in to render or duplicate.</div>}
              </div>
            </div>
          )}
