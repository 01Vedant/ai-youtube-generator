...
import { getBuiltinTemplates, type TemplateItem, getTemplateVars, previewTemplatePlan, renderFromTemplate, startRender } from '@/lib/api';
...
function BuiltinQuickFill({ templateId, onClose }: { templateId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    // Fetch schema/vars here if needed
  }, []);
...
export default function TemplatesPage() {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {selected && (
        <div style={{ marginTop: 24, padding: 24, background: '#f9fafb', borderRadius: 8 }}>
          ...
          {selected.visibility === 'builtin' ? (
            ...
            <button
              ...
              onClick={async () => {
                const res = await startRender({ template_id: selected.id, duration_sec: 10 });
